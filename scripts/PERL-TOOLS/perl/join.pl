...
join.pl goes through each line/key in the LOOKUP_FILE, and finds the *first*
matching key in the DICTIONARY_FILE. The data from those corresponding rows is
then printed out. It does not handle cross-products.
...
produces a new table that is a join of LOOKUP_FILE and DICTIONARY_FILE.

By default, files are assumed to be tab-delimited with the keys in the first
column.
(See the options to change these defaults.)

If LOOKUP_FILE contains the tuple (VOWELS, A, O, A) and DICTIONARY_FILE contains
the tuple (VOWELS, I, U, U) then the joined output will be
(VOWELS, A, O, A, I, U, U)
...
-1 COL: Include column COL from LOOKUP_FILE as part of the key (default is 1).
...
-2 COL: Include column COL from DICTIONARY_FILE as part of the key
         (default is 1).
         See -1 option for discussion of multiple columns.

-o FILLER: Do a left outer join.  If a key in LOOKUP_FILE is not in
            DICTIONARY_FILE, then the tuple from LOOKUP_FILE is printed along
            with the text in FILLER in place of a tuple from DICTIONARY_FILE
            (by default these tuples are not reported in the result).  See -of
            option also to supply FILLER from a file.
...
-e EMPTY: Set the empty string to EMPTY (default is blank).  If both keys
           exist in LOOKUP_FILE and DICTIONARY_FILE but one tuple is blank,
           then the empty character EMPTY will be printed.
...
-neg: Negative output -- print keys that are in LOOKUP_FILE but not in
        DICTIONARY_FILE.
...
-di1 DELIM: Set the input delimiter for LOOKUP_FILE to DELIM (default is tab).

-di2 DELIM: Set the input delimiter for DICTIONARY_FILE to DELIM
              (default is tab).

-di DELIM: Set the input delimiters for both LOOKUP_FILE and DICTIONARY_FILE to
            DELIM (default is tab).
            Equivalent to using both the -di1 and -di2 options.
...
-s1: Suppress printing of tuples from LOOKUP_FILE.  The key is printed, followed
        by the tuple found in DICTIONARY_FILE.

-s2: Suppress printing of tuples from DICTIONARY_FILE.

-sk: Suppress printing of keys.

-r: Reverse
     Instead of printing:
         <key> <tuple_from_LOOKUP_FILE> <tuple_from_DICTIONARY_FILE>,
       print
         <key> <tuple_from_DICTIONARY_FILE> <tuple_from_LOOKUP_FILE>

-u: Uppercase.  (Case-insensitive join)
     Converts non-numeric keys to uppercase, resulting in a case-insensitive
     join.
     Keys from both LOOKUP_FILE and DICTIONARY_FILE will be uppercased before
     attempting the join.
...
join.pl on the input LOOKUP_FILE and DICTIONARY_FILE given below:

LOOKUP_FILE: (tab-delimited)
Alpha   1   2
Alpha   3   4

DICTIONARY_FILE: (tab-delimited)
Alpha   a   first
Alpha   b   middle
Alpha   c   last
